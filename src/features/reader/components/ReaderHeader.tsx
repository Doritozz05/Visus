...
export function ReaderHeader({
  ...
  return (
    <div className={`w-full flex flex-col md:grid md:grid-cols-3 items-center px-4 md:px-8 z-30 border-b border-border/10 pb-3 md:pb-4 gap-2.5 md:gap-4 transition-all duration-300 glass-surface ${isFocusMode ? 'opacity-0 h-0 overflow-hidden py-0 border-0 pointer-events-none' : 'opacity-100 pt-3 md:pt-4'}`}>
      {/* Mobile & Desktop Row container */}
      <div className="flex items-center justify-between w-full md:contents">
        {/* Left: Back to Bookshelf & Tools */}
        <div className="flex items-center justify-start gap-1.5 md:gap-2 min-w-0">
          <button
        ...
        <div className="flex flex-col items-center justify-center min-w-0">
          <h2 className="text-xs font-mono uppercase tracking-widest text-muted-foreground dark:text-foreground/75 font-bold truncate max-w-[95px] xs:max-w-[140px] sm:max-w-xs">
            {activeBook.title}
          </h2>
          <div className="flex items-center gap-1 mt-1 shrink-0">
            ...
            <button
              ...
              className="text-[10px] sm:text-xs text-primary/80 hover:text-primary font-semibold bg-primary/10 hover:bg-primary/20 border border-primary/20 hover:border-primary/45 px-2 py-0.5 rounded flex items-center gap-1 transition-all truncate max-w-[85px] xs:max-w-[110px] sm:max-w-[180px] liquid-glass"
          ...
          <div className="w-24 xs:w-36 sm:w-48 h-1 bg-muted dark:bg-card/90 mt-2 rounded-full overflow-hidden border border-border/40 dark:border-border/20 shadow-inner">
            <div
              className="h-full bg-primary transition-all duration-300"
              style={{ width: `${progressPercentage}%` }}
            />
          </div>
          <span className="text-[9px] sm:text-[10px] font-mono text-muted-foreground/90 dark:text-muted-foreground font-semibold mt-0.5">
      ...
      {/* Mobile Row 2: Mode selector */}
      <div className="w-full flex justify-center md:hidden pt-0.5 border-t border-border/5">
        <FancyTabs
          tabs={[
            { id: "normal", label: "Reader" },
            { id: "rsvp", label: "RSVP" },
          ...
          layoutId="active-reader-mode-mobile"
          variant="pill"
          className="liquid-glass w-full max-w-[280px]"
        />
      </div>
